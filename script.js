...
versionEl.textContent = 'v2.5 (ROI 2x scale + grayscale + contrast stretch + Otsu + morphology)';
...
captureBtn.addEventListener('click', async () => {
  ...
  bigCanvas.width = roiW * scale;
  bigCanvas.height = roiH * scale;
  const bctx = bigCanvas.getContext('2d');
  bctx.imageSmoothingEnabled = true;
  bctx.drawImage(canvas, 0, 0, roiW, roiH, 0, 0, roiW*scale, roiH*scale);

  // 前處理：灰階 → 對比拉伸 → Otsu 二值化 → 形態學開運算
  const processed = preprocessImage(bigCanvas);
  ...
  try {
    ...
    // 顯示結果
    resultDiv.innerHTML = `
      <strong>辨識信心度: ${confidence.toFixed(1)}%</strong><br>
      <strong>原始辨識:</strong><pre>${text}</pre><hr>
